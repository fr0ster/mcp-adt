...
get_structure_source_definition = {
    "name": "get_structure_source",
    "description": "Retrieve source code lines for an ABAP DDIC structure via ADT, with XML and text/plain fallback.",
    "parameters": {
        "type": "object",
        "properties": {
            "structure_name": {
                "type": "string",
                "description": "Name of the ABAP DDIC structure (e.g. 'ZMY_STRUCTURE')."
            }
        },
...
def get_structure_source(
    structure_name: str
) -> list[str]:
    """
    Fetches the source lines of an ABAP DDIC structure via the ADT API.
    ...
    """
    print(f"Fetching structure source for {structure_name}")
    if not structure_name:
        raise ValueError("structure_name is required")

    session = make_session()
    base     = SAP_URL.rstrip('/')
    encoded_structure_name = encode_sap_object_name(structure_name)
    endpoint = (
        f"{base}/sap/bc/adt/ddic/structures/{encoded_structure_name}/source/main"
    )
    ...
    try:
        ...
        if resp.status_code == 404:
            raise AdtError(404, f"Structure '{structure_name}' not found") from e
        raise
